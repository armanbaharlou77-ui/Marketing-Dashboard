...
export default function Login_Step2() {
  ...
  const skipNameDialogSessionClearRef = useRef(false);
  ...
  const handleNameDialogOpenChange = (open) => {
    if (open) {
    ...
    if (skipNameDialogSessionClearRef.current) {
      skipNameDialogSessionClearRef.current = false;
      return;
    }
  ...
  const userRegisterHandler = async () => {
    ...
    const data = await setInfo(firstName, lastName, token);
    if (data.msg === 0) {
      try {
      ...
      skipNameDialogSessionClearRef.current = true;
      setIsNameFormOpen(false);
      ...
      <div className="w-full max-w-md rounded-3xl sm:rounded-4xl border border-white/60 bg-white/80 backdrop-blur-xl p-6 sm:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="mb-4 sm:mb-6 flex justify-start">
          ...
          >
            <div className="bg-slate-100 group-hover:bg-slate-200 p-2 rounded-xl sm:rounded-2xl transition-colors">
              <BsArrowRight
                size={20}
                className="sm:w-6 sm:h-6 text-gray-700 font-extrabold"
              />
            </div>
          </button>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 sm:mt-3 text-sm sm:text-base text-slate-500 leading-relaxed">
            ...
            <span
              style={{ direction: "ltr" }}
              className="inline-block font-bold text-slate-800 tracking-wide"
            >
              {userPhone || "..."}
            </span>{" "}
            را وارد کنید.
        ...
        >
          <div className="space-y-3">
            ...
            <div
              style={{ direction: "ltr" }}
              className="grid grid-cols-4 gap-2 sm:gap-3 w-full sm:w-5/6 mx-auto user-select-none"
            >
              {[0, 1, 2, 3].map((index) => (
                <input
                  key={index}
